...
export const resolveDoubt = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // Mark all matching doubts as resolved for this student/topic pair
        await QueryLog.updateMany(
            { studentId, topic, doubtResolved: { $ne: true } },
            { $set: { doubtResolved: true } }
        );

        res.status(200).json({ success: true, message: 'Doubt marked as resolved.' });
    } catch (error) {
