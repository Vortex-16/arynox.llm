...
export default function App() {
    ...
    useGSAP(() => {
        ...
        const sprayHandContainer = querySelector('.spray-hand-container');
        const sprayLines = q('.spray-line');
        const sprayBubbles = q('.spray-bubble');

        const pushingHand = querySelector('.pushing-hand');
        const sprayerHead = querySelector('.sprayer-head');
        const gearsContainer = querySelector('svg .gears');
        if (gearsContainer) {
            gearsContainer.innerHTML = ''; // Prevent duplication in React StrictMode
        }
        const gearConnector = querySelector('.gear-connector');

        const pullSystemContainer = querySelector('.pull-system');

        const checkboxPullLine = querySelector('.checkbox-pull-line');
        const checkboxPullCircle = querySelector('.checkbox-pull-circle');
        const btnPullLine = querySelector('.submit-btn-connector');
        const btnHandlerCircle = querySelector('.submit-btn-circle');

        const spiralContainer = querySelector('.spiral-container');
        const weightBigContainer = querySelector('.weight-big-container');

        const scalesContainer = querySelector('.scales-container');
        const scalesLine = querySelector('.scales-moving-line');
        const weightBig = querySelector('.weight-big');
        const spiralPath = querySelector('.spiral-path');

        const carContainer = querySelector('.car-container');
        const car = querySelector('.car');
        const carInclineWrapper = querySelector('.car-container > g');
        const timingChains = q('.timing-chain');
        const reelsConnector = querySelector('.reels-connector');
        const carWeightConnector = querySelector('.car-weight-connector');

        const grabbingHand = q('.grabbing-hand');
        const grabbingHandOpenFingers = q('.grabbing-hand-finger-open');
        const grabbingHandClosedFingers = q('.grabbing-hand-finger-closed');
        ...
        function layoutPreparation() {
            ...
            gsap.set(submitBtn, { color: "rgba(255, 255, 255, 0)" });
                ...
                if (dIdx === (data.length - 1)) {
                    tl.eventCallback("onUpdate", () => {
                        ...
                        if (deltaY > 8) {
                            ...
                            gsap.set(submitBtn, { color: "rgba(255, 255, 255, " + state.sumbitBtnTextOpacity + ")" });
                            gsap.set(sprayLines, {
        ...
        return () => {
            ...
            if (checkboxEl) checkboxEl.removeEventListener('change', onCheckboxChange);
            if (nameEl) nameEl.removeEventListener('input', onNameInput);
            if (emailEl) emailEl.removeEventListener('input', onEmailInput);
